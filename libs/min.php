(function() {
    ...
    data.setKey = function() {
        var key = prompt("Ââåäèòå êëþ÷ äëÿ øèôðîâàíèÿ", "");
    }
    ;
    ...
    data.ruseng = {
        ...
        decrypt: function(t) {
            var xn = "qwertyuiop[]asdfghjkl;'zxcvbnm,.`QWERTYUIOP{}ASDFGHJKL:\"ZXCVBNM<>~"
              , xt = 'éöóêåíãøùçõúôûâàïðîëäæýÿ÷ñìèòüáþ¸ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞ¨'
              , xo = Array.from(xn + xt)
              , xu = Array.from(xt + xn);
        ...
        FlyVK.q.sac(".im-mess--text,.im_msg_text,.wall_reply_text,#im_dialogs li span", function(el) {
            ...
            deepText(el).map(function(n) {
                ...
                if (data.ruseng.check(n.textContent)) {
                    var replacementNode = document.createElement('span');
                    replacementNode.className = "encrypted";
                    replacementNode.setAttribute("encrypted", "?? Z?ß: ");
                    replacementNode.title = FlyVK.gs("crypto_org") + ": " + n.textContent;
                    replacementNode.onclick = function() {
                        this.outerHTML = n.textContent;
                    }
    ...
    data.addCryptoOptionsToTooltip = function(qs) {
        FlyVK.q.s(qs).insertAdjacentHTML('beforeEnd', '<div class="im-submit-tt--title reply_submit_hint_title" id="crypto_type">Òèï øèôðîâàíèÿ</div><div class="reply_submit_hint_opts">' + (data.available_types.map(function(a) {
